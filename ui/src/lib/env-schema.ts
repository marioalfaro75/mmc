...
export const ENV_SCHEMA: EnvVarDef[] = [
  ...
  { key: 'WIREGUARD_MTU', label: 'WireGuard MTU', type: 'integer', group: 'vpn', description: 'Tunnel MTU size — default 1420. Lower to 1280 only if you see VPN issues on double-NAT links', default: '1420', affectsServices: VPN_SERVICES },
  ...
  { key: 'WATCHTOWER_TOKEN', label: 'Watchtower API Token', type: 'secret', group: 'services', description: 'Shared secret the dashboard uses to ask Watchtower to pull and recreate a container. Required — media-ui cannot recreate itself, so updates go through Watchtower', sensitive: true, affectsServices: ['watchtower', 'media-ui'] },
  { key: 'WATCHTOWER_SCHEDULE', label: 'Update Schedule', type: 'cron', group: 'services', description: 'Unused. Watchtower runs on demand now — every image is pinned, so a scheduled sweep could only re-pull the same digest. Kept so existing .env files still validate', default: '0 0 4 * * *', affectsServices: ['watchtower'] },
